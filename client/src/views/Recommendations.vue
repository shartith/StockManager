<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-2xl font-bold text-slate-800">추천 종목</h2>
      <button @click="showAdd = true" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 transition">
        + 추천 추가
      </button>
    </div>
    ...
    <div v-if="showAdd" class="fixed inset-0 bg-black/30 flex items-center justify-center z-50" @click.self="showAdd = false">
      <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
        <h3 class="text-lg font-bold text-slate-800 mb-4">추천 종목 추가</h3>
        <form @submit.prevent="addRecommendation" class="space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">종목코드</label>
            ...
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">종목명</label>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">시장</label>
              <select v-model="addForm.market" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">신호</label>
              <select v-model="addForm.signal_type" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div>
            <label class="block text-xs font-medium text-slate-600 mb-1">추천 사유</label>
          ...
          <div>
            <label class="block text-xs font-medium text-slate-600 mb-1">신뢰도 ({{ addForm.confidence }}%)</label>
          ...
          <div class="flex gap-2 pt-2">
            ...
            <button type="button" @click="showAdd = false" class="flex-1 bg-slate-100 text-slate-600 py-2 rounded-lg text-sm hover:bg-slate-200">취소</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-else-if="filteredRecs.length === 0" class="text-center py-16 text-slate-400">
      <p class="text-4xl mb-3">🔍</p>
      <p>{{ selectedMarket === 'ALL' ? '추천 종목이 없습니다' : `${selectedMarket} 추천 종목이 없습니다` }}</p>
    </div>
    <div v-else class="grid gap-4">
      <div v-for="rec in filteredRecs" :key="rec.id"
        class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition">
        <div class="flex items-start gap-4">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1 flex-wrap">
              <span class="font-bold text-slate-800">{{ rec.name }}</span>
              <span class="text-xs font-mono text-slate-400">{{ rec.ticker }}</span>
              <span class="text-xs px-2 py-0.5 rounded-full font-medium"
                :class="rec.market === 'KRX' ? 'bg-blue-50 text-blue-700' : 'bg-green-50 text-green-700'">
                {{ rec.market }}
              </span>
              <span class="text-xs px-2 py-0.5 rounded-full font-medium"
                :class="rec.signal_type === 'BUY' ? 'bg-red-50 text-red-700' : rec.signal_type === 'SELL' ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-600'">
                {{ rec.signal_type === 'BUY' ? '매수' : rec.signal_type === 'SELL' ? '매도' : '관망' }}
              </span>
              <span v-if="rec.score > 0" class="text-xs px-2 py-0.5 rounded-full font-bold"
                :class="rec.score >= 100 ? 'bg-red-100 text-red-700' : rec.score >= 80 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'">
                {{ rec.score }}점
              </span>
            </div>
            <p v-if="rec.reason" class="text-sm text-slate-600 mt-1">{{ rec.reason }}</p>
          ...
          <div class="flex flex-col gap-2 flex-shrink-0">
            ...
            <button @click="dismissRec(rec.id)" class="w-24 text-xs px-3 py-2 bg-slate-50 text-slate-500 rounded-lg hover:bg-slate-100 transition text-center whitespace-nowrap">
              제외
            </button>
          </div>
        </div>
