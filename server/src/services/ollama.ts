...
[기대수익(EV) 모델 — 매수 결정 전 반드시 확인]
- 한국 주식 round-trip 거래 비용: 약 0.40% (위탁수수료 0.015% + 매도세 0.18% + 거래세 0.20% 슬리피지 추정)
- 슬리피지 비용 = 스프레드% × 0.5 (즉시 체결 시 half-spread 부담)
- 총 거래비용% = (스프레드% × 0.5) + 0.40
- BUY 신호의 기대수익이 총 거래비용%를 충분히 상회해야 EV가 양수.
  예: 스프레드 0.6% → 비용 0.70%. 기대수익이 1.5% 미만이면 EV가 작아 BUY를 HOLD로 전환.
- 단기 매매(1~3일 보유)일수록 EV 마진을 더 엄격히 적용.`;
