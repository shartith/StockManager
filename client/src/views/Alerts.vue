<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-2xl font-bold text-slate-800">알림 설정</h2>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="showForm = false">
      <div class="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
        ...
        <form @submit.prevent="addAlert" class="space-y-3">
          ...
          <div>
            <label class="block text-sm text-slate-600 mb-1">알림 유형 *</label>
            <select v-model="form.type" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" required>
          ...
          <div>
            ...
            <input v-model.number="form.value" type="number" step="any" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" required />
          </div>
    ...
    <div class="space-y-3">
      <div v-for="a in alerts" :key="a.id"
        class="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex items-center justify-between">
        <div class="flex items-center gap-4">
          <div class="w-10 h-10 rounded-full flex items-center justify-center text-lg"
            :class="a.is_active ? 'bg-blue-100' : 'bg-slate-100'">
            {{ a.type === 'PRICE_ABOVE' ? '📈' : a.type === 'PRICE_BELOW' ? '📉' : '🎯' }}
          </div>
          <div>
            <p class="font-medium text-slate-800">
              {{ a.ticker }} <span class="text-slate-400 text-xs">{{ a.stock_name }}</span>
            </p>
      ...
      <div v-if="alerts.length === 0" class="text-center py-16 text-slate-400">
