...
KIS API 연동, 외부 OpenAI 호환 LLM 기반 매매 판단, 자동 스케줄링을 지원하는 웹 기반 트레이딩 플랫폼.
...
- **AI 매매 판단** — 외부 OpenAI 호환 LLM (ai.unids.kr / Ollama / OpenAI 등)으로 매수/매도/홀드 결정 (단일 모드 + debate 모드)
- **자동매매** — KRX 스케줄러 기반 자동 매매 및 즉각적인 수동 실행 지원
...
- **히트맵** — 한국 시장 섹터 로테이션 + 시장 breadth 분석
...
| AI | 외부 OpenAI 호환 LLM (ai.unids.kr / Ollama / OpenAI 등) — URL/모델/API 키는 Settings UI에서 구성 |
...
3. **설정 → 외부 LLM 서버** 에서 URL / 모델 / API 키 입력

### 외부 LLM 연동 (v4.13.0+)

v4.13.0부터 로컬 MLX 번들 대신 **외부 OpenAI 호환 엔드포인트**를 사용합니다.
Settings UI 에서 아래 3가지를 입력하세요.

| 필드 | 설명 | 예시 |
|------|------|------|
| `llmUrl` | `/v1` 을 포함한 full base URL | `https://ai.unids.kr/v1` |
| `llmModel` | 서버가 제공하는 모델 ID (새로고침으로 목록 조회 가능) | `gpt-4o-mini`, `llama3.1:8b`, `gemma2:9b` |
| `llmApiKey` | Bearer 토큰 (공개 API 필수, Ollama는 빈 값) | `sk-…` |

지원 예시:
- **ai.unids.kr (기본)**: `https://ai.unids.kr/v1` + API 키
- **Ollama (로컬)**: `http://localhost:11434/v1` + 빈 키 — 별도로 `ollama serve` 실행 필요
- **OpenAI**: `https://api.openai.com/v1` + API 키

기존 Ollama / MLX 잔존물 정리:
- Ollama: `stock-manager --uninstall-ollama`
- MLX venv: `rm -rf ~/.stock-manager/venv`

자세한 이력은 [docs/MLX_MIGRATION.md](docs/MLX_MIGRATION.md) 참고.
...
│       │   ├── llm.ts         # 외부 LLM 연동 (mutex + retry + timeout, OpenAI-호환 API)
