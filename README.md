...
- **AI 매매 판단** — 로컬 Ollama LLM으로 매수/매도/홀드 결정 (단일 모드 + debate 모드)
- **자동매매** — 스케줄러 기반 자동 매매 (KRX/NYSE/NASDAQ)
- **매매 원칙 엔진** — 19가지 룰 기반 신호 검증 (섹터 집중도, 호가 품질, EV 모델 등)
...
- **NAS 자동 동기화** — 외부 백업 (API 키 마스킹) + 로컬 백업 (API 키 포함) 분리
...
| AI | Ollama (로컬 LLM) — 외부 API 키 사용 안 함 |
...
```bash
# tarball 생성
git archive --format=tar.gz --prefix=stock-manager-4.7.5/ -o /tmp/stock-manager-4.7.5.tar.gz v4.7.5

# GitHub CLI로 Release 생성 (gh 설치 필요: brew install gh)
gh release create v4.7.5 /tmp/stock-manager-4.7.5.tar.gz --title "v4.7.5" --notes "변경 내용"
```
...
```ruby
url "https://github.com/shartith/StockManager/releases/download/v4.7.5/stock-manager-4.7.5.tar.gz"
sha256 "새로운_해시값"
version "4.7.5"
# ...
#   Stock Manager v4.7.5
```

```bash
git add -A
git commit -m "Update to v4.7.5"
...
│       │   ├── ollama.ts      # LLM 연동 (mutex + retry + timeout)
